...
sql_create_v = f"create or replace view v_final_kuznetsov_hot as SELECT l.user_id, l.content_id, fkc.content_name, l.start_s,  l.stop_s, fkch.channel_name, l.region_id, fkr.region_name, fkr.code  FROM final_kuznetsov_warm l JOIN final_kuznetsov_content fkc ON l.content_id = fkc.content_id JOIN final_kuznetsov_regions fkr ON l.region_id = fkr.region_id   JOIN final_kuznetsov_channels fkch ON l.channel_id = fkch.channel_id where l.start_s::date between date_trunc('week', current_date)::date - 7 and date_trunc('week',current_date)::date;"


args = {'owner': 'kuznetsov',
        'start_date': datetime(2023, 10, 11), # дата запуска
